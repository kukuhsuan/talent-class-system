    ...
    <div className="mx-auto max-w-6xl p-4 md:p-6">
      <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <select value={year} onChange={(e) => setYear(Number(e.target.value))} className="rounded-lg border border-slate-200 bg-white px-3 py-2">{[2024,2025,2026,2027,2028].map((value) => <option key={value}>{value}</option>)}</select>
          <select value={month} onChange={(e) => setMonth(Number(e.target.value))} className="rounded-lg border border-slate-200 bg-white px-3 py-2">{Array.from({length:12},(_,i)=>i+1).map((value)=><option key={value} value={value}>{value} 月</option>)}</select>
          ...
          <a href={`/api/accounting-month-end?year=${year}&month=${month}&format=xlsx`} className="rounded-lg bg-emerald-600 px-4 py-2 font-bold text-white">完成後下載 Excel</a>
        </div>
      </div>
      ...
      {data && <>
        ...
        <div className="mb-5 grid gap-3 md:grid-cols-4">
          ...
          <Metric label="預計向園所收款" value={money(data.invoices.total)} note={`目前已建立 ${data.invoices.count} 張請款單`} color="emerald" />
        ...
        <div className="mb-5 grid gap-4 lg:grid-cols-[1fr_1.4fr]">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><h2 className="font-black text-slate-900">月底作業順序</h2><div className="mt-3 grid gap-2 sm:grid-cols-2"><Quick href={`/attendance?year=${year}&month=${month}`} title="① 補齊上課人數" note="每堂課都要填實際學生人數"/><Quick href={`/school-invoices?year=${year}&month=${month}`} title="② 建立園所請款單" note="確認要向各園所收多少錢"/><Quick href={`/salary?year=${year}&month=${month}`} title="③ 確認老師薪資" note="核對時數、金額並完成結算"/><Quick href={`/api/accounting-month-end?year=${year}&month=${month}&format=xlsx`} title="④ 下載完整 Excel" note="人數、請款與薪資一次交付會計"/></div></section>
        </div>

        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex items-center justify-between gap-3 border-b border-slate-100 px-4 py-3"><div><h2 className="font-black text-slate-900">園所月結資料</h2><p className="text-xs text-slate-500">課後看學生人次；課內只看時數；Demo 只付老師費用，不向園所請款。</p></div><button onClick={()=>setShowAllSchools((value)=>!value)} className="shrink-0 rounded-lg border border-slate-200 px-3 py-2 text-xs font-bold text-slate-600">{showAllSchools?"只看未完成":"查看全部園所"}</button></div><div className="max-h-[520px] overflow-auto"><table className="w-full min-w-[900px] text-sm"><thead className="sticky top-0 bg-slate-50 text-slate-600"><tr><th className="px-4 py-3 text-left">園所名稱</th><th className="px-4 py-3 text-right">本月課堂</th><th className="px-4 py-3 text-right">課後學生人次</th><th className="px-4 py-3 text-right">課內時數</th><th className="px-4 py-3 text-left">還要處理</th><th className="px-4 py-3 text-center">請款單</th></tr></thead><tbody className="divide-y divide-slate-100">{[...data.schools].filter((row)=>showAllSchools||row.missingCount>0||(row.invoiceRequired&&!row.invoiceCreated)).sort((a,b)=>Number(a.invoiceCreated)-Number(b.invoiceCreated)||b.missingCount-a.missingCount).map((row)=><tr key={row.school}><td className="px-4 py-3 font-semibold text-slate-800">{row.school}</td><td className="px-4 py-3 text-right">{row.lessons} 堂</td><td className="px-4 py-3 text-right">{row.people ? `${row.people} 人次` : "—"}</td><td className="px-4 py-3 text-right">{row.inSchoolHours ? `${row.inSchoolHours} 小時` : "—"}</td><td className="px-4 py-3">{row.missingCount > 0 ? <Link href={`/attendance?year=${year}&month=${month}`} className="font-bold text-amber-700 hover:underline">{row.missingCount} 堂課後課未填人數 →</Link>:<span className="text-emerald-700">✓ 會計資料完成</span>}</td><td className="px-4 py-3 text-center">{row.invoiceRequired?<Link href={`/school-invoices?year=${year}&month=${month}`} className={`rounded-full px-2 py-1 text-xs font-bold ${row.invoiceCreated?"bg-emerald-100 text-emerald-700":"bg-amber-100 text-amber-700"}`}>{row.invoiceCreated?"✓ 已建立":"去建立 →"}</Link>:<span className="rounded-full bg-violet-100 px-2 py-1 text-xs font-bold text-violet-700">Demo 免請款</span>}</td></tr>)}</tbody></table></div></section>
      </>}
    </div>
  );
}
