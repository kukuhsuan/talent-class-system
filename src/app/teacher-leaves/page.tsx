...
export default function TeacherLeavesPage() {
  ...
  async function approve(id: number) {
    try {
      ...
      await action(`/api/teacher-leaves/${id}/approve`, "已核准請假；請按「選老師發詢問」挑選候選老師，或使用「手動指定代課」");
    } catch (error) {
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="space-y-4 border-b border-slate-100 px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">老師可在 LINE 申請，行政也可代為建立。「選老師發詢問」會依老師目前綁定的 LINE 官方帳號送出；月份以請假課程日期計算，不是申請時間。</p>
          </div>
