...
function friendlyError(error: unknown, fallback: string) {
  ...
  return /SERVER_ERROR|HTTP status 502|fetch failed|AbortError|signal is aborted/i.test(message)
...
export default function CourseChangeRequestsPage() {
  ...
  const loadOptions = useCallback(async () => {
    ...
    try {
    const response = await request("/api/course-change-requests/options", { cache: "no-store" });
  ...
  const loadItems = useCallback(async () => {
    ...
    const response = await request(`/api/course-change-requests?${params}`, { cache: "no-store" });
    const body = await response.json();
  ...
  async function save() {
    ...
    try {
      ...
      const response = await request(editingId ? `/api/course-change-requests/${editingId}` : "/api/course-change-requests", {
  ...
  async function action(item: ChangeRequest, name: "send" | "apply" | "cancel" | "return") {
    ...
    setMessage(""); setActionBusy(busyKey);
    try {
      ...
      try { await loadItems(); setMessage(successMessage); }
      catch { setMessage(`${successMessage}；列表暫時無法更新，請稍後重新整理頁面`); }
    } catch (error) {
      setMessage(friendlyError(error, "操作失敗"));
    } finally {
      setActionBusy("");
    }
  }
  ...
  async function arrangeSubstitute(item: ChangeRequest) {
    if (actionBusy) return;
    setActionBusy(`substitute:${item.id}`);
    try {
      const response = await request(`/api/course-change-requests/${item.id}/arrange-substitute`, { method: "POST" });
      const body = await response.json().catch(() => ({}));
      if (!response.ok) { setMessage(friendlyError(body.error, "建立代課安排失敗")); return; }
      window.location.href = "/teacher-leaves";
    } catch (error) {
      setMessage(friendlyError(error, "建立代課安排失敗"));
    } finally {
      setActionBusy("");
    }
  }
    ...
    <div className="mx-auto max-w-7xl p-4 md:p-6">
      ...
      {showForm && <section className="mb-6 rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:p-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          {selected && <div className="md:col-span-2 rounded-lg bg-slate-50 p-3 text-sm leading-6 text-slate-600">園所：{selected.school}｜老師：{selected.teacherName}｜地址：{selected.address || "未填寫"}｜原地點：{selected.location || "未填寫"}{(selected.isPayrollLocked || selected.completed) && <span className="ml-2 font-bold text-rose-600">{selected.blockReason || "此堂已鎖薪或完成"}，不可申請</span>}</div>}
        ...
        <div className="mt-5 flex gap-2"><button disabled={saving || !form.attendanceId || Boolean(selected && (selected.isPayrollLocked || selected.completed))} onClick={save} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{saving ? "儲存中…" : editingId ? "更新申請" : "建立申請"}</button><button onClick={resetForm} className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600">取消</button></div>
      </section>}
      ...
      <section className="rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="divide-y divide-slate-100">{items.map((item) => <article key={item.id} className="p-4 md:p-5">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {["待行政審核", "老師無法配合", "需要討論"].includes(item.status) && <button disabled={Boolean(actionBusy)} onClick={() => action(item, "send")} className="rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">{actionBusy === `send:${item.id}` ? "發送中…" : "發送詢問給老師"}</button>}
            {item.status === "老師可配合" && <button disabled={Boolean(actionBusy)} onClick={() => action(item, "apply")} className="rounded-lg bg-emerald-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">{actionBusy === `apply:${item.id}` ? "套用中…" : "確認並套用異動"}</button>}
            {item.status === "老師無法配合" && <button disabled={Boolean(actionBusy)} onClick={() => arrangeSubstitute(item)} className="rounded-lg bg-rose-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">{actionBusy === `substitute:${item.id}` ? "建立中…" : "安排代課"}</button>}
            {item.requestSource === "SCHOOL" && item.status === "待行政審核" && <button disabled={Boolean(actionBusy)} onClick={() => action(item, "return")} className="rounded-lg border border-amber-200 px-3 py-2 text-sm font-semibold text-amber-700 disabled:opacity-50">{actionBusy === `return:${item.id}` ? "退回中…" : "退回補充"}</button>}
            {!['已完成', '已取消'].includes(item.status) && <button disabled={Boolean(actionBusy)} onClick={() => action(item, "cancel")} className="rounded-lg border border-rose-200 px-3 py-2 text-sm font-semibold text-rose-600 disabled:opacity-50">{actionBusy === `cancel:${item.id}` ? "取消中…" : "取消申請"}</button>}
          </div>
