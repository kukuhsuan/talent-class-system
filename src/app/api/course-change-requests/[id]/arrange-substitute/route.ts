...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const request = await withDatabaseRetry(() => getCourseChangeRequest(Number(id)));
    if (!request) return NextResponse.json({ error: "找不到課程異動申請" }, { status: 404 });
    if (request.status !== "老師無法配合") throw new Error("只有老師無法配合時可以安排代課");
    const leave = await withDatabaseRetry(() => createLeaveRequestFromAttendance({
    ...
    await writeAuditLog(req, {
      ...
      sensitive: true,
    }).catch((error) => console.error("arrange substitute audit failed", error));
    return NextResponse.json({ ok: true, leaveRequestId: leave.id });
  } catch (error) {
    return NextResponse.json({ error: databaseErrorMessage(error, "建立代課安排失敗") }, { status: 400 });
  }
}
