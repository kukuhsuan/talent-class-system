...
export function renderTemplate(body: string, vars: Record<string, string>) {
  return body.replace(/\{(姓名|園所|課程|日期|星期|時間|地址|課程摘要|園所連結|開課確認連結|停課狀態)\}/g, (_, key: string) => vars[key] ?? "");
}
...
export async function buildBatchMessages(opts: BuildOptions): Promise<BatchRecipientMessage[]> {
  ...
  if (opts.targetType === "teacher") {
    ...
    return ids.map((id) => {
      ...
      return { id, name: t.name, lineUserId: t.lineUserId, lineRegion: t.lineRegion || "north", message: finalizeMessage(body, vars) };
    });
  }
